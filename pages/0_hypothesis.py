...
c1, c2 = st.columns(2)
with c1:
    col = st.number_input('Columns:', min_value=2, max_value=10, value=3, step=1)
    ...
    tmp = st.text_area('Re-define column names (separated by , ): ', user_def_col, height=80)
    col_names = str(tmp).split(',')

with c2:
    row = st.number_input('Rows:', min_value=2, max_value=10, value=2, step=1)
    row_names = ["row{}".format(i+1) for i in range(int(row))]
    # row_names = ["row1", "row2"]
    user_def_row = ",".join(row_names)
    tmp = st.text_area('Re-define row names (separated by , ): ', user_def_row, height=80)
